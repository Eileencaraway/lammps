/* ----------------------------------------------------------------------
   ...
   // v(t+dt)        = invDamp*f(t)/m + activeMag*director(t+dt)
   // r(t+dt)        = r(t) + dt*v(t+dt)
   // omega(t+dt)    = invDampRot*Torque(t)/I + noise
   // f(t+dt) and T(t+dt) [compute in Verlet.cpp]
   //*********************************************************************************
  ...
  FixNVE(lmp, narg, arg), random(NULL)
{
  ...
  ssrm << "# invDamp, activeMAG, noiseMag, seed " << Damp << "\t" << activeMag << "\t"
  << noiseMag << "\t" << seed << "\n";
...
void FixBrownianActiveSphere::initial_integrate(int vflag)
{
  ...
  // A = sqrt(3)
  // psi(n) = 2*sqrt(3)*n
  double noisePref  = sqrt(12.0*noiseMag/dtv);
  /*****************************************************/
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) {
          ...
          oneByDamp = ForcePref/(radius[i]*rmass[i]);
          vnoisePref= noisePref/sqrt(6*radius[i]);
          ...
          oneByDampTorque = torquePref/(rmass[i]*r5);
          rnoisePref = noisePref/sqrt(8*r3);

          omega[i][0] = oneByDampTorque*torque[i][0] + rnoisePref*(random->uniform()-0.5);
          omega[i][1] = oneByDampTorque*torque[i][1] + rnoisePref*(random->uniform()-0.5);
          omega[i][2] = oneByDampTorque*torque[i][2] + rnoisePref*(random->uniform()-0.5);
