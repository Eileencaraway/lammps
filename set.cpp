...
enum{TYPE,TYPE_FRACTION,MOLECULE,X,Y,Z,CHARGE,MASS,SHAPE,LENGTH,TRI,
     ...
     MESO_E,MESO_CV,MESO_RHO,SMD_MASS_DENSITY,SMD_CONTACT_RADIUS,INAME,DNAME,RAT};
...
void Set::command(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    ...
    } else if (strstr(arg[iarg],"rat") == arg[iarg]) { 
      fraction = force->numeric(FLERR,arg[iarg+1]); // probability of being in state 0
      t_run = force->numeric(FLERR,arg[iarg+2]); //running timescale
      t_tumble = force->numeric(FLERR,arg[iarg+3]); //running timescale
      ivalue = force->inumeric(FLERR,arg[iarg+4]); // seed
      setrandom(RAT); 
      iarg += 5;
    } else error->all(FLERR,"Illegal set command");
...
void Set::setrandom(int keyword)
{
  ...
  // set quaternions to random orientations in 3d or 2d
    ...
    for(i = 0; i < nlocal; i++)
      if(select[i]){ 
        random->reset(seed,x[i]);
        if (random->uniform() > fraction){ // tumbling state
          atom->state[i] = 0;
          atom->t_transition[i] = -int(t_tumble);
        }else{
          atom->state[i] = 1;
          atom->t_transition[i] = -int( -1.0/t_run*log(random->uniform() ));
        }
        count++;
      }
